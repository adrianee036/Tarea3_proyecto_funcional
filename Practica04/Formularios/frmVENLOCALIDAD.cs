...
{ 
    public partial class frmVENLOCALIDAD : Form
    {

        public string varf1;  // variable publica usada para transporta la data asignada en ella hacia el formulario padre
        public string varf2;

        public frmVENLOCALIDAD()
        {
            InitializeComponent();
        }

        private void frmVENLOCALIDAD_Load(object sender, EventArgs e)
        {
            this.KeyPreview = true;     // puedo presionar la tecla escape
            this.Text = HMenu.cia + "   Consulta Localidades";

            this.dgv.EnableHeadersVisualStyles = false;
            this.dgv.ColumnHeadersVisible = true;
            this.dgv.RowHeadersVisible = false;

            this.dgv.Columns.Add("Col00", "ID de Localidad");
            this.dgv.Columns.Add("Col01", "Nombre de la Localidad");
        ...
        private void frmVENLOCALIDAD_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void BuscarDatos()
        {
            ...
            SqlCommand sqCmd = new SqlCommand("SELECT IDLocalidad, NombreLocalidad"
                                              + "  FROM Localidad "
                                              + " WHERE NombreLocalidad LIKE '%" + txtBuscar.Text + "%'"
                                              + " ORDER BY NombreLocalidad ASC", conexion);
